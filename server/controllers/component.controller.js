...
export const publishComponent = async (req, res) => {
    try {
        ...
        //this help to to 2 setp back ../virtual-ui-lib and in that we have 3 main folder and process.cwd will give you the path of the there folder located
        // ~ publishComponent ~ libPath: D:\shdcn component\library
        const libPath = path.join(process.cwd(), "../library")
        console.log("🚀 ~ publishComponent ~ libPath:", libPath)

        //this if for creating folder inside the components
        const componentDir = path.join(
            libPath,
            "src/components",
            component.name
        )
        console.log("🚀 ~ publishComponent ~ componentDir:", componentDir)

        //this is for creating the file inside the folder
        const componentFile = path.join(
            componentDir,
            `${component.name}.jsx`
        );
        //finding index js file 
        ...
        // prevent duplicate export exportline does not include inside indexContent
        ...
        // UPDATE VERSION
        // -----------------------------
        // changing the version in package.json file then only we canpublish in every publish unquie name shoule be there
        ...
        component.npmPackage = "virtual-ui-library01"
...
export const generateComponent = async (req, res) => {
    try {
        ...
        res.json({
            parsed, remainingCredits: user.role === "user" ? user.aiCredits : null,
        })
