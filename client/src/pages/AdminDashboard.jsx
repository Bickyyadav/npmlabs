...
const AdminDashboard = () => {
    ...
    const handleLogout = () => {
        try {
            axios.get(import.meta.env.VITE_SERVER_URL + `/api/auth/logout`, { withCredentials: true });
    ...
    const chartData = (() => {
        ...
        return Object.entries(map).map(([date, count]) => ({ date, components: count })).sort((a, b) => new Date(a.date) - new Date(b.date))
            .slice(-12)
    })()
    ...
    const filteredPublicComponents = componentSearch.trim() ? publicComponents.filter((c) => c.name?.toLowerCase().includes(componentSearch.toLowerCase()) || c.props?.some((p) => p.toLowerCase().includes(componentSearch.toLowerCase()))) : publicComponents
    ...
    const AddComponentForm = ({ onSaved }) => {
        ...
        const handleSave = async () => {
            ...
            setSaving(true);
            try {
                const res = await axios.post(import.meta.env.VITE_SERVER_URL + `/api/component/save`,
        ...
        const handlePublish = async () => {
            ...
            setPublishing(true);
            try {
                await axios.post(import.meta.env.VITE_SERVER_URL + `/api/component/publish`,
                    { componentId: savedId },
                    { withCredentials: true })
                setIsPublished(true);
        ...
        const PropsInput = ({ props, setProps }) => {
